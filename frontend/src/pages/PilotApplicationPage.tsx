...
export function PilotApplicationPage(){
  ...
  return <main className="min-h-screen bg-[#0a0f1e] px-4 py-8 text-slate-100 sm:px-8"><div className="mx-auto max-w-3xl">
    ...
    <header className="py-8"><p className="text-xs font-semibold uppercase tracking-widest text-cyan-300">Build around your reporting workflow</p><h1 className="mt-3 text-3xl font-bold">Request free early access</h1><p className="mt-4 leading-relaxed text-slate-300">Tell us how your centre reports today and what slows your team down. We’ll use these details to confirm workspace access, usage capacity and any integration requirements. Product use has no subscription fee during early access; no credit card is required.</p><p className="mt-3 text-sm text-slate-400">No account required. Share workflow details only—no patient data, reports, passwords or API keys.</p></header>
    {receipt?<section role="status" className="rounded-xl border border-emerald-800 bg-emerald-950/30 p-6"><h2 className="text-xl font-semibold">Application received</h2><p className="mt-3">{receipt.message}</p><p className="mt-4 break-all text-sm text-slate-300">Reference: {receipt.reference}</p><p className="mt-3 text-sm text-slate-400">Your request is saved. Workspace access and usage capacity will be confirmed separately. No payment or software installation has been initiated.</p><Link className="mt-5 inline-block text-cyan-300 underline" to="/demo">Try the clinical demo →</Link></section>:
    <form onSubmit={submit} className="rounded-xl border border-slate-700 bg-slate-900 p-5 sm:p-7">
      ...
      <fieldset disabled={busy} className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="sm:col-span-2">Main workflow problem<textarea className={input} required maxLength={5000} rows={4} placeholder="What takes the most time or causes the most rework? What would a useful evaluation improve?" value={form.mainProblem} onChange={e=>change('mainProblem',e.target.value)} /></label>
        ...
        <label className="flex items-start gap-3 text-sm text-slate-300 sm:col-span-2"><input type="checkbox" required className="mt-1" checked={form.contactConsent} onChange={e=>change('contactConsent',e.target.checked)} /><span>I agree that Med-AI may store these application details and contact me about early access. This does not subscribe me to marketing emails.</span></label>
        <button className="rounded-lg bg-cyan-300 px-5 py-3 font-semibold text-slate-950 disabled:opacity-50 sm:col-span-2" type="submit">{busy?'Submitting…':'Request free access'}</button>
      </fieldset>
    </form>}
