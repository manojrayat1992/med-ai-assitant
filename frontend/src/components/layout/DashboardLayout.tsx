  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg, #0a0f1e)' }}>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        {/* ── Top Header ── */}
        <header
          className="relative z-50 flex h-14 shrink-0 items-center justify-between px-6 gap-4"
          style={{
            position: 'relative',
            zIndex: 50,
        ...
        >
          ...
          <div className="flex items-center gap-3 min-w-0">
            {isFullWidth ? (
              <Link to="/dashboard" className="flex items-center gap-2 shrink-0">
                ...
                <span className="hidden text-sm font-bold sm:inline" style={{ color: 'var(--clr-text)' }}>Med-AI</span>
              </Link>
            ) : (
              <div className="hidden sm:block">
                <h2 className="text-sm font-bold leading-none" style={{ color: 'var(--clr-text)' }}>{meta.title}</h2>
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <div ref={searchRef} style={{ position: 'relative' }} className="hidden md:block">
              ...
              >
                ...
                {query ? (
                  ...
                  <kbd
                    ...
                    style={{ background: 'var(--surface-3, #1e2a40)', color: 'var(--clr-text-3)' }}
                  >⌘K</kbd>
                )}
              </div>
              ...
              {searchOpen && (
                <div style={{
                  ...
                  boxShadow: '0 16px 48px rgba(0,0,0,0.4)', zIndex: 999,
                  minWidth: 280,
                }}>
                  {searching && (
            ...
            <div ref={userMenuRef} className="relative ml-1" style={{ zIndex: 60 }}>
              <button
                type="button"
              ...
              >
                ...
                <div className="hidden text-left leading-tight lg:block">
                  <p className="text-xs font-semibold" style={{ color: 'var(--clr-text)' }}>{fullName ?? 'Clinical User'}</p>
                  <p className="text-[10px] capitalize" style={{ color: 'var(--clr-text-3)' }}>{role ? role.replace(/_/g, ' ').toLowerCase() : 'Practitioner'}</p>
                </div>
                <ChevronDown className="h-3.5 w-3.5" style={{ color: 'var(--clr-text-3)' }} />
              </button>
              ...
              {userMenuOpen && (
                ...
                >
                  ...
                  <Link
                    ...
                    className="block px-3.5 py-2.5 text-xs font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/5" style={{ color: 'var(--clr-text-2)' }}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => void logout()}
                    className="flex w-full items-center gap-2 border-t px-3.5 py-2.5 text-left text-xs font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/5 hover:text-red-500"
                    style={{ color: 'var(--clr-text-2)', borderColor: 'var(--clr-border, #1e2d45)' }}
                  >
        ...
        {/* ── Main Content ── */}
        <main
          className="relative z-0 flex-1 overflow-y-auto p-6"
